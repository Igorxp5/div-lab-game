...
class RoundResultWindow(QMainWindow):
    def __init__(self):
        ...
        #texto nº de jogadores eliminados
        self.text = QLabel(self)
        self.text.setText('Jogadores eliminados:')
        self.text.setWordWrap(True)
        self.text.setStyleSheet("font: 15pt") 
        self.text.move(250, 200)   
        self.text.setMinimumSize(1000, 32)

        #Lista de jogadores eliminados da sala
        # ----- COLOCAR ÍCONE AO LADO DE QUEM ESTA ASSISTINDO
        # ----- ESSA MESMA LISTA SERVIRÁ PARA VOTAR EM UM ORGANIZADOR DA PARTIDA
        eliminatedPlayersList = myListWidget(self)
        eliminatedPlayersList.addItem("Jogador 1")
        #listView.clicked.connect() add função de clique
        eliminatedPlayersList.move(250, 260)
        eliminatedPlayersList.resize(300, 150)
...
if __name__ == '__main__':
    ...
    roundResultWindownWin = RoundResultWindow()
    roundResultWindownWin.show()
    sys.exit( app.exec_() )